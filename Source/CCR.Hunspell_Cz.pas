...
resourcestring
  SCouldNotAddCustomWord = 'Ќе удалось добавить пользовательское слово ''%s''';
  SCouldNotAddCustomWordWithAffix = 'Ќе удалось добавить пользовательское слово ''%s'' с аффиксом ''%s''';
  SHunspellDLLAleadyLoaded = 'Hunspell DLL уже загружена';
  SHunspellDLLNotFound = 'Hunspell DLL не найдена';
  SHunspellHandleNeeded = 'ќпераци€ требует чтобы сначала был загружен словарь';
...
begin
  Result := CheckDLLInitialize(True, Name);
end;
